...
function init(){
 ...
 const token=()=>typeof savedEntitlements==='function'?savedEntitlements()[active?.course]?.token:null;
 const tokenOwner=()=>{try{const body=token().split('.')[0].replace(/-/g,'+').replace(/_/g,'/');return String(JSON.parse(atob(body)).code_id);}catch(_){return '';}};
