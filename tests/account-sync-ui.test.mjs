...
assert.equal(document.querySelector('#savedAccountSync').hasAttribute('open'),false);assert.equal(document.querySelectorAll('[role=tab]').length,2);document.querySelector('[data-tab=guest]').onclick();assert.equal(document.querySelector('#syncStudentPanel').hidden,true);document.querySelector('[data-tab=student]').onclick();assert.equal(document.querySelector('#syncGuestPanel').hidden,true);assert.equal(requests,0,'no cloud call before opt-in');assert.equal(JSON.parse(storage.getItem(core.WORDS_KEY))[0].hanzi,'猫');
...
await document.querySelector('[data-connect]').onclick();assert.equal(opened,0);assert.equal(document.querySelector('#savedAccountSync').open,true);assert.equal(document.querySelector('[data-student-form]').hidden,false);
