...
(async () => {
  ...
  document.body.classList.add('shadow-study-mode'); ctx.renderShadowLive(0);
  assert.ok($('shadowLive').querySelector('.sentence-save'), 'save is available inside course Shadowing');
  click($('shadowLive').querySelector('.sentence-save')); assert.equal(rows().length, 0, 'course Shadowing uses the same saved-sentence library');
  click($('shadowLive').querySelector('.sentence-save')); assert.equal(rows().length, 1, 'course Shadowing can save the current sentence');
  document.body.classList.remove('shadow-study-mode');
  ...
  console.log('PASS: real-renderer DOM integration — sentence controls in standalone and course Shadowing, no playback bubbling/nested buttons, persistence, duplicates, corrupt/quota storage, source filtering, authenticated replay, locked replay, Back.');
