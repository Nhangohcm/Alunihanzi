...
s="import { savedLibraryRoute } from './saved-library.mjs';\nimport { accountsRoute, accountSession } from './accounts.mjs';\n"+s.replace(anchor,"    const accountResponse=await accountsRoute(req,env,headers);\n    if(accountResponse)return accountResponse;\n    const savedResponse=await savedLibraryRoute(req,env,{getBearerPayload,accountSession,headers});\n    if(savedResponse)return savedResponse;\n"+anchor)
a.output.mkdir(parents=True)
(a.output/'index.js').write_text(s)
shutil.copyfile(Path(__file__).with_name('worker-module.mjs'),a.output/'saved-library.mjs')
shutil.copyfile(Path(__file__).with_name('accounts.mjs'),a.output/'accounts.mjs')
shutil.copyfile(Path(__file__).with_name('email-accounts.mjs'),a.output/'email-accounts.mjs')
